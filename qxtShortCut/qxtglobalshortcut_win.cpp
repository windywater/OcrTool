    ...
    void * message, long * result)
{
	...
	if (msg->message == WM_SYSKEYDOWN
		&& msg->wParam == VK_F4)
	{
		activateShortcut(VK_F4, modifiers);
		return true;
	}
	//
...
{
    return RegisterHotKey((HWND)winId, nativeMods ^ nativeKey, nativeMods, nativeKey);
}

bool QxtGlobalShortcutPrivate::unregisterShortcut(quint32 nativeKey, quint32 nativeMods)
{
    return UnregisterHotKey((HWND)winId, nativeMods ^ nativeKey);
}
